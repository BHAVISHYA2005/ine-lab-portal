...
export default function Submissions() {
  ...
  return <main className="page-frame"><div className="page-heading"><div><span className="eyebrow">YOUR WORK</span><h1>Submission history.</h1><p className="lede">Review what you have sent and what to improve next.</p></div></div>
    ...
    {!loading && !error && submissions.length > 0 && <div className="submission-list">{submissions.map((submission) => <Card key={submission.id} className="submission-row"><div><span className="eyebrow">{submission.lab_title ?? `Lab #${submission.lab_id}`}</span><p className="muted submission-date">{new Date(submission.submitted_at).toLocaleString()}</p></div><span className={`status status-${submission.status}`}>{submission.status}</span></Card>)}</div>}
  </main>;
}
